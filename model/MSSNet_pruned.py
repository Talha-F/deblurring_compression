...
layer_filters = [39, 39, 46, 16, 19, 10, 10, 46, 54, 19, 22, 12, 12, 46, 22, 54, 39, 54, 14, 19, 8, 10, 46, 46, 16, 16, 10,
                 10, 46, 19, 39, 46, 16, 22, 8, 10, 39, 39, 14, 16, 10, 8, 39, 19, 54, 39, 54, 16, 19, 10, 10, 39, 46, 16,
                 16, 10, 8, 46, 16, 46, 46, 19, 22, 10, 10, 46, 39, 16, 19, 10, 8, 46, 16, 39, 46, 22, 26, 8, 10, 46, 46,
                 19, 19, 10, 10, 46, 19]

itr = 0
##########################################################################
def conv(in_channels, out_channels, kernel_size, bias=False, stride = 1):
    #return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size, padding=(kernel_size//2), bias=bias, stride = stride, groups = in_channels))
...
class ResBlock(nn.Module):
    def __init__(self, wf, kernel_size, reduction, bias, act):
        ...
        modules_body = []
        global itr
        modules_body.append(conv(wf, layer_filters[itr], kernel_size, bias=bias))
        modules_body.append(act)
        modules_body.append(conv(layer_filters[itr], wf, kernel_size, bias=bias))
        itr += 1
        print(itr)
...
class DeblurNet(nn.Module):
    ...
    def forward(self, s3_blur):
        ...
        return [res1_1,res2_1,res2_2,res3_1,res3_2,res3_3]
